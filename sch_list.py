...
async def get_sch_detail(sch_meta:dict, cust_header:dict=None, verify:bool=False):
    async with httpx.AsyncClient(headers=cust_header, verify=verify, timeout=None) as client:
        ...
        data = {
            ...
            'tgl_sk_pendirian': l_establish_date,
            'no_sk_ijop': l_opr_id,
...
def get_dataset(subdistricts:list, w_units:list, get_detail:bool, batch_limit:int=400, cust_header:dict=None, ssl_verif:bool=False) -> dict:
    sch_list_crawl = [get_sch_list(subdist, w_unit, cust_header, ssl_verif) for subdist in subdistricts for w_unit in w_units]
    crawl_exec = asyncio.run(asyncio.gather(*sch_list_crawl))
    sch_list = [sch for sch_sublist in crawl_exec for sch in sch_sublist]
    match get_detail:
        case False:
            return sch_list
        case True:
            throttle = [sch_list[sch:sch+batch_limit] for sch in range(0, len(sch_list), batch_limit)]
            data = []
            for subset in throttle:
